import { useEffect, useState } from "react";
import styled from "styled-components";
import { firebaseDB } from "firebaseInstance";
import { doc, getDoc } from "firebase/firestore";

interface ImageWrapperProps {
  width: string;
  height: string;
}

interface ImgProps {
  width: string;
  height: string;
}

const StyledProfileWrapper = styled.div`
  width: 100%;
  height: 70px;
  display: flex;
  align-items: center;
  border-bottom: 1px solid #f8f8f8;
`;

const StyledProfile = styled.div`
  width: 100%;
  height: 50px;
  display: flex;
`;

const StyledImageWrapper = styled.div<ImageWrapperProps>`
  width: ${(props) => props.width};
  height: ${(props) => props.height};
  display: flex;
  align-items: center;
`;

const StyledImg = styled.img<ImgProps>`
  width: ${(props) => props.width};
  height: ${(props) => props.height};
  border-radius: 50%;
`;

const InfoWrapper = styled.div`
  display: flex;
  justify-content: center;
  flex-direction: column;
  gap: 5px;
`;

const StyledNickname = styled.div`
  font-weight: bold;
  font-size: 14px;
`;

const StyledMessage = styled.div`
  font-size: 11px;
  color: #b4b4b4;
`;

const StyledFriends = styled.div`
  width: 100%;
  height: 470px;
`;

const StyledFriendsCounter = styled.div`
  width: 100%;
  height: 20px;
  margin-top: 10px;
  color: #7e7e7e;
  font-size: 11px;
  font-weight: bold;
`;

const StyledFriendsList = styled.ul`
  width: 100%;
  height: 100%;
  display: flex;
  flex-direction: column;
  gap: 10px;
`;

const StyledFriendsItem = styled.ul``;

interface MyProfileProps {
  uid: string;
}

const MyProfile = ({ uid }: MyProfileProps) => {
  const [myProfile, setMyProfile] = useState<any>({});
  const [friends, setFriends] = useState<any>([]);

  useEffect(() => {
    const getUser = async () => {
      const docRef = doc(firebaseDB, "users", uid);
      try {
        const docSnap = await getDoc(docRef);
        const userData = docSnap.data();
        const userObj = {
          nickname: userData?.nickname,
          message: userData?.message,
          profile: userData?.profile,
        };
        setMyProfile(userObj);
      } catch (error: any) {
        console.error(error);
      }
    };

    const getFriend = async () => {
      const docRef = doc(firebaseDB, "users", uid);
      const docSnap = await getDoc(docRef);
      const userFriends = [...docSnap.data()?.friends];

      for (const userFriend of userFriends) {
        const docRef = doc(firebaseDB, "users", userFriend);
        const docSnap = await getDoc(docRef);
        const friendInfo = docSnap.data();
        setFriends((prev: any) => [friendInfo, ...prev]);
      }
    };

    getUser();
    getFriend();
  }, []);

  return (
    <>
      <StyledProfileWrapper>
        <StyledProfile>
          <StyledImageWrapper width="60px" height="100%">
            {myProfile.profile ? (
              <StyledImg src={myProfile.profile} width="50px" height="100%" />
            ) : (
              <StyledImg
                width="50px"
                height="100%"
                src="data:image/jpeg;base64,"
              />
            )}
          </StyledImageWrapper>
          <InfoWrapper>
            <StyledNickname>{myProfile.nickname}</StyledNickname>
            <StyledMessage>{myProfile.message}</StyledMessage>
          </InfoWrapper>
        </StyledProfile>
      </StyledProfileWrapper>
      <StyledFriends>
        <StyledFriendsCounter>친구 {friends.length}</StyledFriendsCounter>
        <StyledFriendsList>
          {friends.map((friend: any) => (
            <StyledFriendsItem>
              <StyledProfile>
                <StyledImageWrapper width="50px" height="100%">
                  {myProfile.profile ? (
                    <StyledImg
                      src={friend.profile}
                      width="40px"
                      height="40px"
                    />
                  ) : (
                    <StyledImg
                      width="40px"
                      height="40px"
                      src="data:image/jpeg;base64,"
                    />
                  )}
                </StyledImageWrapper>
                <InfoWrapper>
                  <StyledNickname>{friend.nickname}</StyledNickname>
                  <StyledMessage>{friend.message}</StyledMessage>
                </InfoWrapper>
              </StyledProfile>
            </StyledFriendsItem>
          ))}
        </StyledFriendsList>
      </StyledFriends>
    </>
  );
};

export default MyProfile;
